...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i, dim in enumerate(hidden_dims):
        self.params['W'+str(i+1)] = np.random.normal(0.0, weight_scale, (rows, dim))
        self.params['b'+str(i+1)] = np.zeros(dim,)
        rows = dim
    self.params['W'+str(len(hidden_dims)+1)] = np.random.normal(0.0, weight_scale, (rows, num_classes))
  ...
  def loss(self, X, y=None):
    ...
    for layer in range(1,self.num_layers,1):
        out, cache = affine_relu_forward(out, self.params['W'+str(layer)], self.params['b'+str(layer)])
        caches.append(cache)
    ...
    ############################################################################
    loss, dx = softmax_loss(scores, y)
    loss = loss + 0.5 * self.reg * sum([np.sum(p[1]**2) for p in self.params.items()])
    
    dx, grads['W'+str(self.num_layers)], grads['b'+str(self.num_layers)] = affine_backward(dx, caches[-1])
    grads['W'+str(self.num_layers)] += self.reg * self.params['W'+str(self.num_layers)]
    for layer in range(self.num_layers-1,0,-1):
        dx, grads['W'+str(layer)], grads['b'+str(layer)] = affine_relu_backward(dx, caches[layer-1])
