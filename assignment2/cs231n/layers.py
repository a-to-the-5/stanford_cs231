...
def affine_forward(x, w, b):
  ...
  #############################################################################
  X = x.reshape(x.shape[0], reduce((lambda x, y: x * y), x.shape[1:]))
  out = X.dot(w) + b
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
    ...
    for f in range(F):
      ...
      for i in range(hdash):
        for j in range(wdash):
          out[n,f,i,j] = np.sum(fil*padded[n,:,i*stride:i*stride+HH,j*stride:j*stride+WW]) + b[f]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, w, b, conv_param)
...
def conv_backward_naive(dout, cache):
  ...
  #############################################################################
  padded = np.pad(x, ((0,0),(0,0),(pad,pad),(pad,pad)), 'constant', constant_values=0)
  dx, dw, db = np.zeros(padded.shape), np.zeros(w.shape), np.sum(dout, axis = (0,2,3))
  hdash = 1 + (H + 2 * pad - HH) / stride
  wdash = 1 + (W + 2 * pad - WW) / stride
  for n in range(N):
    for f in range(F):
      ...
      for i in range(hdash):
        for j in range(wdash):
          dx[n,:,i*stride:i*stride+HH,j*stride:j*stride+WW] += fil*dout[n,f,i,j]
          dw[f] += padded[n,:,i*stride:i*stride+HH,j*stride:j*stride+WW]*dout[n,f,i,j]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def max_pool_backward_naive(dout, cache):
  ...
  for n in range(N):
    for c in range(C):
      for i in range(hdash):
        for j in range(wdash):
          currx = x[n,c,i*stride:i*stride+pool_height,j*stride:j*stride+pool_width]
          dx[n,c,i*stride:i*stride+pool_height,j*stride:j*stride+pool_width] = (currx == np.max(currx))*dout[n,c,i,j]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
